...
namespace DAL
{
    ...
    public partial class Dal_TB_BCBZ
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select A.C_ID,A.D_START,A.C_PRO_ID,A.D_END,B.C_BC_NAME,C.C_BZ_NAME,A.C_EMP_ID,A.D_MOD_DT,C_GZ ");
  ...
  FROM XGERP50.PD_PGA WHERE 1=1");
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetLastList(string pro, DateTime et)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select A.C_ID,A.D_START,A.C_PRO_ID,A.D_END,B.C_BC_NAME,C.C_BZ_NAME,A.C_EMP_ID,A.D_MOD_DT ");
            strSql.Append(" FROM TB_BCBZ A,TB_BC B,TB_BZ C");
            strSql.Append(" WHERE A.C_BC=B.C_ID AND A.C_BZ=C.C_ID AND A.C_PRO_ID='" + pro + "'  AND A.D_START<=to_date('" + et + "','yyyy-MM-dd HH24:mi:ss')");
            strSql.Append(" ORDER BY D_START desc");
            return DbHelperOra.Query(strSql.ToString());
        }
        /// <summary>
        /// 获得当前规则
        /// </summary>
        public string GetGZNEW(string pro)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_GZ ");
            strSql.Append(" FROM TB_BCBZ");
            strSql.Append(" WHERE C_PRO_ID='" + pro + "'  AND D_START<=to_date('" + DateTime.Now + "','yyyy-MM-dd HH24:mi:ss') AND D_END>=to_date('" + DateTime.Now + "','yyyy-MM-dd HH24:mi:ss')");
            ...
            {
                return dt.Rows[0]["C_GZ"].ToString();
            }
        }
