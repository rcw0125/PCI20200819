...
namespace DAL
{
    ...
    public partial class Dal_TB_STA
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select    T.c_pro_id,T1.C_PRO_DESC from TB_STA t INNER JOIN tb_pro t1  ON t.c_pro_id=t1.c_id WHERE t.n_status=1 AND t1.c_pro_code IN ('XM','KP','ZX','CC') AND T.C_STA_DESC NOT LIKE '%抛丸%' GROUP BY T.C_PRO_ID, T1.C_PRO_DESC   "); 
            return DbHelperOra.Query(strSql.ToString());
        }
        /// <summary>
        /// 获得数据列表
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  t.c_pro_id,T.c_sta_code, t.c_sta_desc from TB_STA t INNER JOIN tb_pro t1  ON t.c_pro_id=t1.c_id WHERE t.n_status=1 AND t1.c_pro_code IN ('XM','KP','ZX','CC') AND T.C_STA_DESC NOT LIKE '%抛丸%' and t.c_pro_id ='" + c_sta_code + "'  ORDER BY t1.c_pro_code,T.N_SORT ");
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetStaListNotZX(string strWHERE)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT t.* ");
            strSql.Append(" FROM TB_STA t  where t.n_status=1 and t.C_STA_DESC  ");
            if (strWHERE.Trim() != "")
            {
                strSql.Append(" and " + strWHERE);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT C_PRO_ID from TB_STA ");
            strSql.Append(" where C_STA_CODE=:C_STA_CODE  AND N_STATUS=1");
            OracleParameter[] parameters = {
                    new OracleParameter(":C_STA_CODE", OracleDbType.Varchar2,100)          };
            parameters[0].Value = C_STA_CODE;
