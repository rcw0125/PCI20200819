...
namespace DAL
{
    ...
    /// 可以用户可以修改满足自己项目的需要。
    /// </summary>
    public abstract class DbHelperNC
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.	
        public static string connectionString = "Data Source=192.168.2.12/NCV5;User Id=PCIQUERY;Password=PCIQUERY;enlist=dynamic";
        ...
        /// <param name="parameters">存储过程参数</param>
        /// <returns></returns>
        public static string RunProcedureOut(string storedProcName, IDataParameter[] parameters)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                string result;
                ...
                result = command.Parameters["P_MSG"].Value.ToString();
                connection.Close();
                return result;
            }
        }
