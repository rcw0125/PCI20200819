...
namespace DAL
{
    public partial class Dal_Interface_NC_DMXCSJ
    {
        public Dal_Interface_NC_DMXCSJ()
        { }
        ...
        public DataSet GetZJBList(string dhstr)
        {
            ...
            strSql.Append("select C_ID,C_DISPATCH_ID,C_SEND_STOCK,N_NUM,C_CKDH,C_LIC_PLA_NO,N_WGT,N_JZ,C_STOVE,C_PLAN_ID,C_STL_GRD,C_STD_CODE,C_SPEC,C_PK_NCID,C_MZDATE,N_MWGT,N_PWGT,N_MZTIME,N_PZTIME,C_PZDATE");
        ...
        /// <returns></returns>
        public bool SendXml_DM(string xmlFileName, string fydh)
        {
            try
            {
                ...
                root.SetAttribute("filename", "" + fydh + ".xml");
                ...
                CreateNode(xmlDoc, head, "apprdate", "");
                ...
                CreateNode(xmlDoc, head, "pk_delivbill_h", modDispatch.C_GUID);//发运单表头主键
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (modDayPlan != null)
                        {
                            ...
                            decimal num = Convert.ToDecimal(dt.Rows[i]["N_WGT"]);//数量
                            ...
                            CreateNode(xmlDoc, item, "ccheckstate_bid", modDayPlan.C_VFREE4);//质量等级**
                            ...
                            CreateNode(xmlDoc, item, "dinvassist", dt.Rows[i]["N_NUM"].ToString());//辅数量-件数
                            CreateNode(xmlDoc, item, "dinvnum", num.ToString());//存货数量
                            ...
                            CreateNode(xmlDoc, item, "nassistnetwgt", "");
                            CreateNode(xmlDoc, item, "nfeedbacknum", num.ToString());//回写上游数量
                            CreateNode(xmlDoc, item, "nnetwgt", dt.Rows[i]["N_JZ"].ToString());//净重
                            CreateNode(xmlDoc, item, "nquoteunitnum", num.ToString());//报价计量单位数量？-存货数量
                            ...
                            CreateNode(xmlDoc, item, "pkarriveaddress", modOrder.C_RECEIVEADDRESS);//到货地点
                            CreateNode(xmlDoc, item, "pkarrivearea", modOrder.C_RECEIPTAREAID);//到货地区主键
                            ...
                            CreateNode(xmlDoc, item, "pksendstock", "");
                            ...
                            CreateNode(xmlDoc, item, "vbatchcode", dt.Rows[i]["C_BATCH_NO"].ToString());
                            ...
                            CreateNode(xmlDoc, item, "vcustcode", "");//******?
                            CreateNode(xmlDoc, item, "vcustname", modCust.C_NAME);//收货单位名称
                            CreateNode(xmlDoc, item, "vdayplancode", modDayPlan.C_PLCODE);//对应日计划单据号
                            CreateNode(xmlDoc, item, "vdestaddress", modDayPlan.C_DESTADDRESS);//收货地址
                            CreateNode(xmlDoc, item, "vdestarea", modArea.C_NAME);//收货地区
                            ...
                            CreateNode(xmlDoc, item, "vfree3", modDayPlan.C_VFREE3);//包装
