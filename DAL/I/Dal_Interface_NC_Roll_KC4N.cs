...
namespace DAL
{
    public partial class Dal_Interface_NC_Roll_KC4N
    {
        private Dal_Send_NC dalSendNC = new Dal_Send_NC();
        Dal_TRC_ROLL_PRODCUT dal_roll_pro = new Dal_TRC_ROLL_PRODCUT();
        Dal_TQC_ROLL_COMMUTE dal_roll_commute = new Dal_TQC_ROLL_COMMUTE();
        ...
        /// <returns></returns>
        public string SendXml_GP(string xmlFileName, string c_master_id, string c_gp_before_id, string c_gp_after_id)
        {
            try
            {
                string urlname = "XC" + c_master_id + ".xml";
                string url = xmlFileName + "\\NCXML";
                if (!Directory.Exists(url))
                {
                    Directory.CreateDirectory(url);
                }
                Mod_TQC_ROLL_COMMUTE mod_roll_commute = dal_roll_commute.GetModel_Interface_Trans(c_master_id, c_gp_before_id, c_gp_after_id); //改判记录
                if (mod_roll_commute == null) return "改判失败，获取改判记录时错误！";
                DataTable dt = dal_roll_commute.GetList_Interface_Trans(c_master_id, c_gp_before_id, c_gp_after_id).Tables[0];
                Mod_TRC_ROLL_PRODCUT mod_roll_pro = dal_roll_pro.GetModel_Iterface_Trans(mod_roll_commute.C_ROLL_ID); //实绩主表
                if (mod_roll_pro == null) return "改判失败，获取主键失败！";
                Mod_TB_MATRL_MAIN mod_mater_main_before = dal_mater_main.GetModel_Interface_Trans(mod_roll_commute.C_MAT_CODE_BEFORE); //改判前物料主表
                Mod_TB_MATRL_MAIN mod_mater_main_after = dal_mater_main.GetModel_Interface_Trans(mod_roll_commute.C_MAT_CODE_AFTER); //改判后物料主表

                Mod_TS_USER mod_ts_user = dal_user.GetModel_Interface_Trans(mod_roll_commute.C_EMP_ID);//用户主表
                ...
                Mod_TPB_LINEWH mod_linewh = dal_linewh.GetModel_Interface_Trans(mod_roll_pro.C_LINEWH_CODE);//库存表
         
                Mod_TMO_ORDER mod_tmo_order = dal_tmo_order.GetModelByORDERNO_Trans(mod_roll_pro.C_ORDER_NO);//订单池
                ...
                Mod_TQB_CHECKSTATE mod_checkstate_before = dal_checkstate.GetModelByName_Trans(mod_roll_commute.C_JUDGE_LEV_BP_BEFORE.ToString(), strXGID);//改判前判定等级
                Mod_TQB_CHECKSTATE mod_checkstate_after = dal_checkstate.GetModelByName_Trans(mod_roll_commute.C_JUDGE_LEV_BP_AFTER.ToString(), strXGID);//改判后判定等级
                if (dt == null) return "改判失败，获取主键失败！";
                if (mod_mater_main_before == null) return "改判失败，获取改判前物料编码错误！";
                if (mod_mater_main_after == null) return "改判失败，获取改判后物料编码错误！";
                if (mod_ts_user == null) return "改判失败，获取用户信息错误！";
                if (mod_linewh == null) return "改判失败，获取库存信息错误！";
                if (mod_checkstate_before == null) return "改判失败，获取改判前判定等级信息错误！";
                if (mod_checkstate_after == null) return "改判失败，获取改判后判定等级信息错误！";
                ...
                CreateNode(xmlDoc, head, "ts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                CreateNode(xmlDoc, head, "timestamp", "");
                CreateNode(xmlDoc, head, "headts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                ...
                CreateNode(xmlDoc, head, "tlastmoditime", mod_roll_commute.D_COMMUTE_DATE.ToString());//最后修改时间
                ...
                CreateNode(xmlDoc, item, "scrq", Convert.ToDateTime(mod_roll_pro.D_MOD_DT).ToString("yyyy-MM-dd"));
                CreateNode(xmlDoc, item, "sjpdzq", "");
                CreateNode(xmlDoc, item, "vbatchcode", mod_roll_pro.C_BATCH_NO);//vbatchcode
                CreateNode(xmlDoc, item, "vfree0", "");//自由项1
                CreateNode(xmlDoc, item, "vfree1", mod_roll_commute.C_ZYX1_BEFORE);
                CreateNode(xmlDoc, item, "vfree2", mod_roll_commute.C_ZYX2_BEFORE);
                CreateNode(xmlDoc, item, "vfree3", mod_roll_commute.C_BZYQ_BEFORE);
                ...
                CreateNode(xmlDoc, item, "ts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                CreateNode(xmlDoc, item, "timestamp", "");
                CreateNode(xmlDoc, item, "bodyts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                ...
                CreateNode(xmlDoc, item, "vnote", "");//备注
                CreateNode(xmlDoc, item, "tchecktime", mod_roll_pro.D_JUDGE_DATE.ToString());
                ...
                CreateNode(xmlDoc, item, "cqualitylevelname", mod_roll_commute.C_JUDGE_LEV_BP_BEFORE);
                CreateNode(xmlDoc, item, "vdef1", "");
                CreateNode(xmlDoc, item, "vdef2", "");
                CreateNode(xmlDoc, item, "vdef3", "");
                ...
                CreateNode(xmlDoc, item_gp, "scrq", Convert.ToDateTime(mod_roll_pro.D_MOD_DT).ToString("yyyy-MM-dd"));
                CreateNode(xmlDoc, item_gp, "sjpdzq", "");
                CreateNode(xmlDoc, item_gp, "vbatchcode", mod_roll_pro.C_BATCH_NO);//vbatchcode
                CreateNode(xmlDoc, item_gp, "vfree0", "");//自由项1
                CreateNode(xmlDoc, item_gp, "vfree1", mod_roll_commute.C_ZYX1_AFTER);
                CreateNode(xmlDoc, item_gp, "vfree2", mod_roll_commute.C_ZYX2_AFTER);
                CreateNode(xmlDoc, item_gp, "vfree3", mod_roll_commute.C_BZYQ_AFTER);
                ...
                CreateNode(xmlDoc, item_gp, "ts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                CreateNode(xmlDoc, item_gp, "timestamp", "");
                CreateNode(xmlDoc, item_gp, "bodyts", mod_roll_commute.D_COMMUTE_DATE.ToString());
                ...
                CreateNode(xmlDoc, item_gp, "vnote", "");//备注
                CreateNode(xmlDoc, item_gp, "tchecktime", mod_roll_pro.D_JUDGE_DATE.ToString());
                ...
                CreateNode(xmlDoc, item_gp, "cqualitylevelname", mod_roll_commute.C_JUDGE_LEV_BP_AFTER);
                CreateNode(xmlDoc, item_gp, "vdef1", "");
                CreateNode(xmlDoc, item_gp, "vdef2", "");
                CreateNode(xmlDoc, item_gp, "vdef3", "");
