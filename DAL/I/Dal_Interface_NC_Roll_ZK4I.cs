...
namespace DAL
{
    public partial class Dal_Interface_NC_Roll_ZK4I
    {
        ...
        public string SendXml_GP(string xmlFileName,string id)
        {
            try
            {
                string name = "4I" + id + ".xml";
                xmlFileName += "\\NCXML\\" + name;
                DataTable zkddt = dal_TRC_ROLL_ZKD.GetListBydh(id).Tables[0];
                if (zkddt.Rows.Count==0)
                {
                    return "PCI转库单表未查询到转库单："+ id+"信息！";
                }
                DataRow dtrow = zkddt.Rows[0];
                Mod_TPB_LINEWH yck = dal_TPB_LINEWH.GetModel(dtrow["C_LINEWH_CODE"].ToString());//源仓库
                Mod_TPB_LINEWH mbck = dal_TPB_LINEWH.GetModel(dtrow["C_MBLINEWH_CODE"].ToString());//目标仓库
                ...
                root.SetAttribute("filename", "ZK4I"+ id + ".xml");
                ...
                CreateNode(xmlDoc, head, "vbillcode",id);	//单据号
                ...
                foreach (DataRow row in zkddt.Rows)
                {
                    ...
                    Mod_TQB_CHECKSTATE mod_TQB_CHECKSTATE = dal_TQB_CHECKSTATE.GetModelByName(row["C_JUDGE_LEV_ZH"].ToString(), "1001");
                    Mod_TRC_ROLL_ZKD mod_TRC_ROLL_ZKD = dal_TRC_ROLL_ZKD.GetModel(row["C_ID"].ToString());
                    ...
                    CreateNode(xmlDoc, item, "dshldtransnum", mod_TRC_ROLL_ZKD.N_WGT.ToString());   //应转数量
                    CreateNode(xmlDoc, item, "dvalidate", "");
                    CreateNode(xmlDoc, item, "fbillrowflag", "");
                    CreateNode(xmlDoc, item, "hlzl", "");
                    CreateNode(xmlDoc, item, "hsl", Math.Round(Convert.ToDouble(mod_TRC_ROLL_ZKD.N_WGT / mod_TRC_ROLL_ZKD.N_NUM), 6).ToString());   //换算率
                    ...
                    CreateNode(xmlDoc, item, "nshldtransastnum", mod_TRC_ROLL_ZKD.N_NUM.ToString());    //应转数量
                    ...
                    CreateNode(xmlDoc, item, "nminstocknum", mod_TRC_ROLL_ZKD.N_NUM.ToString());    //？
                    CreateNode(xmlDoc, item, "norderpointnum", mod_TRC_ROLL_ZKD.N_WGT.ToString());  //？
                    ...
                    CreateNode(xmlDoc, item, "cqualitylevelname", mod_TRC_ROLL_ZKD.C_JUDGE_LEV_ZH); //？
                    CreateNode(xmlDoc, item, "vdef1", "");  //？
                    CreateNode(xmlDoc, item, "vdef2", "");  //？
                    CreateNode(xmlDoc, item, "vdef3", "");  //？
                    #endregion
                ...
                //C: \Users\14472\Desktop\20181012xml\库存
