...
namespace DAL
{
    public partial class Dal_Interface_NC_SLAB_GP4A
    {
        ...
        public string SendXml_GP4A(string xmlFileName, string dh, string dhid, int num, string C_SLABWH_LOC_CODE, string C_SLABWH_AREA_CODE)
        {
            try
            {
                string urlname = "GP4A" + dh + ".XML";//XML名称
                ...
                DataTable dt = dal_TSC_SLAB_MAIN.Get_RK_List(dh, dhid, num).Tables[0];
                DataRow dr = null;
                if (dt.Rows.Count > 0)
                {
                    dr = dt.Rows[0];
                ...
                Mod_TS_USER mod_TS_USER = dal_TS_USER.GetModel(RV.UI.UserInfo.userID);
                string bmid = dal_TS_DEPT.GetDept(mod_TS_USER.C_ACCOUNT);
                if (bmid == "")
                {
                    return "未找到部门信息";///"操作人部门未维护！";
                }
                ...
                if (mod_TB_MATRL_MAIN == null) return "未找到物料信息";
                if (yck == null) return "未找到原仓库信息";
                if (mbck == null) return "未找到目标仓库信息";
                if (mod_TS_USER == null) return "未找到用户信息";
                if (mod_TQB_CHECKSTATE == null) return "质量等级获取失败";
                if (modSms == null) return "钢坯实绩信息获取失败";
                ...
                {
                    DataTable dtLOC = dal_TPB_SLABWH.GetList_NC_Loc(C_SLABWH_LOC_CODE, mbck.C_SLABWH_CODE, C_SLABWH_AREA_CODE).Tables[0];
                ...
                CreateNode(xmlDoc, item, "hsl", Math.Round(Convert.ToDouble(dr["wgt"]) / Convert.ToDouble(dr["qua"]), 6).ToString("0.000000"));	//换算率
