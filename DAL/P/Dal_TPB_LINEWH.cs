...
namespace DAL
{
    ...
    public partial class Dal_TPB_LINEWH
    {
        ...
        public object GetList_id(string C_LINEWH_CODE)
        {
            ...
            strSql.Append(" FROM TPB_LINEWH where 1=1 and C_LINEWH_CODE ='" + C_LINEWH_CODE + "'");
        ...
        public string GetNCID(string ckcode)
        {
            string sql = "select pk_stordoc,storcode,storname from xgerp50.bd_stordoc@CAP_ERP where pk_corp='1001'  and dr=0 and  storcode='" + ckcode + "'";
            DataTable dt = DbHelperOra.Query(sql).Tables[0];
            if (dt.Rows.Count > 0)
            {
                string ncid = dt.Rows[0]["pk_stordoc"].ToString();
                return ncid;
            }
            else
            {
        ...
        public string TBNCID(string ckcode,string ncid)
        {
            string upsql = "update tpb_LINEWH set C_ID='" + ncid + "' WHERE C_LINEWH_CODE='" + ckcode + "' AND N_STATUS=1";
            if (DbHelperOra.ExecuteSql(upsql) > 0)
            {
                return "1";
            }
            else
