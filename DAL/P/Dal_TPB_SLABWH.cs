    ...
    public partial class Dal_TPB_SLABWH
    {
        ...
        public bool Add(Mod_TPB_SLABWH model, bool IsID)
        {
            ...
            if (IsID)
            {
                ...
                strSql.Append("C_ID,C_SLABWH_CODE,C_SLABWH_NAME,D_END_DATE,C_REMARK,C_EMP_ID)");
                ...
                parameters[2].Value = model.C_SLABWH_NAME;
        ...
        public DataSet GetList()
        {
            ...
            strSql.Append(" FROM TPB_SLABWH WHERE N_STATUS=1 ORDER BY to_number(C_SLABWH_CODE) ");
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetList_WW()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select C_ID,C_LINEWH_CODE,C_LINEWH_NAME FROM tpb_linewh ");
            strSql.Append(" WHERE N_STATUS=1 and C_LINEWH_CODE like '681%' ORDER BY to_number(C_LINEWH_CODE)  ");
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetList_KP()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_ID,C_SLABWH_CODE,C_SLABWH_NAME");
            strSql.Append(" FROM TPB_SLABWH WHERE N_STATUS=1 ");
            strSql.Append("  and C_SLABWH_NAME like '%开坯%' ");
            strSql.Append(" ORDER BY to_number(C_SLABWH_CODE) ");
        ...
        /// <returns></returns>
        public bool UpdateRemark(string stove, string remark)
        {
            string sql = "UPDATE TSC_SLAB_MOVE T SET T.C_REMARK = '" + remark + "'    WHERE T.C_STOVE = '" + stove + "'   ";
            string sql1 = " UPDATE TSC_SLAB_MAIN T SET T.C_REMARK = '" + remark + "'  WHERE T.C_STOVE = '" + stove + "' ";
            int rows = DbHelperOra.ExecuteSql(sql);
            int rows1 = DbHelperOra.ExecuteSql(sql1);
            if (rows1 > 0)
            {
                return true;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet GetList_NC_Loc(string C_SLABWH_LOC_CODE, string C_SLABWH_CODE, string C_SLABWH_AREA_CODE)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TC.C_ID AS LOC_ID,TC.C_SLABWH_LOC_CODE,TC.C_SLABWH_LOC_NAME FROM TPB_SLABWH TA INNER JOIN TPB_SLABWH_AREA TB ON TA.C_ID=TB.C_SLABWH_ID INNER JOIN TPB_SLABWH_LOC TC ON TC.C_SLABWH_AREA_ID=TB.C_ID WHERE TA.C_SLABWH_CODE IN ('132','593') AND TA.N_STATUS=1 AND TB.N_STATUS=1 AND TC.N_STATUS=1 AND TC.C_SLABWH_LOC_CODE='" + C_SLABWH_LOC_CODE + "' AND TA.C_SLABWH_CODE='" + C_SLABWH_CODE + "' AND TB.C_SLABWH_AREA_CODE='" + C_SLABWH_AREA_CODE + "' ");
