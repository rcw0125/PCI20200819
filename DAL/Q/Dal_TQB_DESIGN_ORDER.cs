    ...
    public partial class Dal_TQB_DESIGN_ORDER
    {
        ...
        public bool Add(Mod_TQB_DESIGN_ORDER model)
        {
            ...
            OracleParameter[] parameters = {
                    ...
                    new OracleParameter(":C_EMP_ID_BG", OracleDbType.Varchar2,100),
                    new OracleParameter(":D_MOD_DT_BG", OracleDbType.Date),
                    new OracleParameter(":C_EMP_ID_SH", OracleDbType.Varchar2,100),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ta.C_ID,ta.C_DELIVERY_STATE,tc.c_design_no as 质量设计号,td.c_std_code as 执行标准,tb.C_NO as 订单号,tb.c_stl_grd as 钢种,tb.c_mat_code as 物料编码,tb.c_mat_name as 物料名称, (select t.c_name from TS_USER t where t.c_id=ta.c_emp_id_bg and t.n_type=3)as 变更人,ta.d_mod_dt_bg as 变更时间,(select t.c_name from TS_USER t where t.c_id=ta.c_emp_id_sh and t.n_type=3)as 审核人,ta.d_mod_dt_sh as 审核时间,decode(ta.n_status,0,'停用',1,'可用',2,'待审核')as 状态  from TQB_DESIGN_ORDER ta inner join tmo_condetails tb on ta.c_order_id=tb.c_no inner join tqb_design tc on ta.c_design_id=tc.c_id inner join tqb_std_main td on td.c_id=tc.c_std_main_id where ta.n_status in (1,2) ");
        ...
        public DataSet GetDesignByStdGrd(string strStdcode, string strGrd)
        {
            ...
            if (strStdcode.Trim() != "")
            {
                strSql.Append(" and tb.c_std_code = '" + strStdcode + "' ");
            }

            if (strGrd.Trim() != "")
            {
                strSql.Append(" and tb.c_stl_grd = '" + strGrd + "' ");
            }
