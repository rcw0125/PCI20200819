...
{
    /// <summary>
    /// 数据访问类:TQC_COMPRE_SLAB
    /// </summary>
    public partial class Dal_TQC_COMPRE_SLAB
    {
        public Dal_TQC_COMPRE_SLAB()
        { }
        #region  BasicMethod
        ...
        public bool Exists(string C_ID)
        {
            ...
            strSql.Append("select count(1) from TQC_COMPRE_SLAB");
        ...
        public bool Delete(string C_ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from TQC_COMPRE_SLAB ");
            strSql.Append(" where C_ID=:C_ID ");
            OracleParameter[] parameters = {
                    new OracleParameter(":C_ID", OracleDbType.Varchar2,100)         };
            parameters[0].Value = C_ID;
        ...
        public DataSet GetList(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_ID,C_STOVE,C_BATCH_NO,C_STL_GRD,N_WGT,N_QUA,C_STD_CODE,C_SPEC,C_MAT_CODE,C_MAT_NAME,C_SHIFT,C_GROUP,C_RESULT_FACE,C_REASON_NAME,C_REASON_CODE,D_DP_DT,C_RESULT_ELE,C_RESULT_PHY,C_RESULT_ALL,D_RESALL_DT,C_RESULT_PEOPLE,D_RESPEO_DT,C_QR_STATE,C_EMP_ID,D_MOD_DT,C_RESULT_PHYSEC,C_RESULT_PHYFINAL,C_DESIGN_NO,N_STATUS,C_REMARK ");
            strSql.Append(" FROM TQC_COMPRE_SLAB ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from TQC_COMPRE_SLAB T ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" SELECT * FROM (SELECT t.C_ID,T.C_STOVE AS 炉号, T.C_STL_GRD AS 钢种, T.C_SPEC AS 断面, T.C_STD_CODE AS 执行标准, T.C_MAT_CODE AS 物料编码, T.C_MAT_NAME AS 物料名称, T.N_LEN AS 长度, T.N_QUA AS 件数, T.N_WGT AS 重量, T.C_RESULT_FACE AS 表判结果, T.C_REASON_NAME AS 不合格原因, T.C_RESULT_ELE AS 成分结果, T.C_RESULT_ALL AS 综合判定结果, T.C_RESULT_PEOPLE AS 人工判定结果, T.D_RESALL_DT AS 判定时间,T.C_QR_STATE AS 是否确认, T.C_EMP_ID, T.D_MOD_DT AS 确认时间, T.D_DP_DT AS 生产时间, T.C_DESIGN_NO AS 质量设计号,t.C_REMARK as 备注 FROM TQC_COMPRE_SLAB T where N_STATUS=1 ");
            ...
            strSql.Append(" ORDER BY T.D_DP_DT desc )TC LEFT JOIN(select listagg(TT.C_BATCH_NO, ',') within group(order by tt.C_STOVE)AS 开坯号, TT.C_STOVE,TT.C_STA_DESC from(SELECT DISTINCT TM.C_BATCH_NO, TM.C_STOVE,tm.c_sta_desc FROM TSC_SLAB_MAIN TM WHERE 1=1");

            if (stove.Trim() != "")
            {
                strSql.Append(" and tM.C_STOVE like '" + stove + "%' ");
            }

            if (strTime1.Trim() != "" && strTime2.Trim() != "")
            {
                strSql.Append(" and tM.D_CCM_DATE between to_date('" + strTime1 + "','yyyy-mm-dd hh24:mi:ss') and to_date('" + strTime2 + "','yyyy-mm-dd hh24:mi:ss') ");
            }

            strSql.Append(" ) TT group by tt.c_stove,TT.C_STA_DESC) TL ON TC.炉号 = TL.C_STOVE ");
        ...
        public DataSet GetList_CX(string stoveMin, string stoveMax, string strTime1, string strTime2, string strState, string strGZ, string strBZ)
        {
            ...
            if (stoveMin.Trim() != "")
            {
                strSql.Append(" and T.C_STOVE >='" + stoveMin + "' ");
            }

            if (stoveMax.Trim() != "")
            {
                strSql.Append(" and T.C_STOVE <='" + stoveMax + "' ");
            }

            if (strGZ.Trim() != "")
            {
                strSql.Append(" and T.C_STL_GRD like '%" + strGZ + "%' ");
            }

            if (strBZ.Trim() != "")
            {
                strSql.Append(" and T.C_STD_CODE like '%" + strBZ + "%' ");
            }
            ...
            strSql.Append(" ORDER BY T.D_DP_DT desc )TC LEFT JOIN(select listagg(TT.C_BATCH_NO, ',') within group(order by tt.C_STOVE)AS 开坯号, TT.C_STOVE from(SELECT DISTINCT TM.C_BATCH_NO, TM.C_STOVE FROM TSC_SLAB_MAIN TM WHERE 1=1");

            if (stoveMin.Trim() != "")
            {
                strSql.Append(" and tM.C_STOVE >='" + stoveMin + "' ");
            }

            if (stoveMax.Trim() != "")
            {
                strSql.Append(" and tM.C_STOVE <='" + stoveMax + "' ");
            }

            if (strGZ.Trim() != "")
            {
                strSql.Append(" and tM.C_STL_GRD like '%" + strGZ + "%' ");
            }

            if (strBZ.Trim() != "")
            {
                strSql.Append(" and tM.C_STD_CODE like '%" + strBZ + "%' ");
            }
            ...
            strSql.Append(" ) TT group by tt.c_stove) TL ON TC.炉号 = TL.C_STOVE ");
        ...
        /// 获取表判记录
        /// </summary>
        public int Get_MES_BP_Count(string stove)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT COUNT(1) FROM CQA_BLOOM_HOTJUDGE_DATA@cap_mes T WHERE T.HEATID='" + stove + "' ");
        ...
        /// 删除一条数据
        /// </summary>
        public bool Delete_MES_BP_Trans(string stove)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from CQA_BLOOM_HOTJUDGE_DATA@cap_mes where HEATID='" + stove + "' ");
        ...
        /// 获取表判记录
        /// </summary>
        public int Get_MES_ZP_Count(string stove)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT COUNT(1) FROM CQA_BLOOM_FINJUDGE_DATA@cap_mes T WHERE T.HEATID='" + stove + "' ");
        ...
        /// 删除一条数据
        /// </summary>
        public bool Delete_MES_ZP_Trans(string stove)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from CQA_BLOOM_FINJUDGE_DATA@cap_mes where HEATID='" + stove + "' ");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update_LGJH_Trans(string strLGJH, string stove)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update CBLOOM_DATA@cap_mes set FINAL_STEELGRADEINDEX='" + strLGJH + "',STATUS='9' WHERE MATERIALID='" + stove + "' ");
        ...
        /// 获取成分记录
        /// </summary>
        public int Get_MES_CF_Count(string APPID, string SAMPLE_CODE, string HEATID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT count(1) FROM CQA_LAB_ELEMENT@cap_mes T WHERE UNITTYPEID='6' and APPID='" + APPID + "' and SAMPLE_CODE='" + SAMPLE_CODE + "' and HEATID='" + HEATID + "' ");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update_CF_Trans(string APPID, string SAMPLE_CODE, string HEATID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("UPDATE CQA_LAB_ELEMENT@cap_mes T SET T.FINALJUDGEFLAG='1' WHERE UNITTYPEID='6' and APPID='" + APPID + "' and SAMPLE_CODE='" + SAMPLE_CODE + "' and HEATID='" + HEATID + "' ");
        ...
        {
            return DbHelperOra.RunProcedure("PKG_COMPRE_SLAB.P_JUDGE_SLAB_ZD");
        }
        ...
        public string JUDGE_SLAB_STOVE(string P_STOVE)
        {
            ...
            return DbHelperOra.RunProcedureOut("PKG_COMPRE_SLAB.P_JUDGE_SLAB_STOVE", parameters);
        }
