...
namespace DAL
{
    ...
    public partial class Dal_TS_CUSTFILE
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_NO,C_NAME,C_SHORNAME,C_AREAMMAX");
            strSql.Append(" FROM TS_CUSTFILE where N_STATUS=1 ");
            if (C_NO.Trim() != "")
            {
                strSql.Append(" AND  C_NO LIKE '%"+ C_NO + "%' " );
            }
            if (C_NAME.Trim() != "")
            {
                strSql.Append(" AND  C_NAME LIKE '%" + C_NAME + "%' ");
            ...
            if (C_AREAMMAX.Trim() != "")
            {
                strSql.Append(" AND  C_AREAMMAX LIKE '%" + C_AREAMMAX + "%' ");
            }
            return DbHelperOra.Query(strSql.ToString());
        }
