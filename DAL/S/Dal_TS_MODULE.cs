...
namespace DAL
{
    ...
    /// 数据访问类:TS_MODULE
    /// </summary>
    public partial class Dal_TS_MODULE
    {
        public Dal_TS_MODULE()
        { }
        ...
        public bool Exists(string C_MODULEID)
        {
            ...
            OracleParameter[] parameters = {
                    new OracleParameter(":C_MODULEID", OracleDbType.Varchar2,100)            };
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from ts_user_fun_pci ");
            strSql.Append(" where c_module_id='" + c_module_id + "' ");
        ...
        /// 得到一个对象实体
        /// </summary>
        public Mod_TS_MODULE DataRowToModel(DataRow row)
        {
            Mod_TS_MODULE model = new Mod_TS_MODULE();
            if (row != null)
            {
        ...
        public DataSet GetList_query(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_MODULEID,C_PARENTMODULEID,C_MODULENAME,C_ASSEMBLYNAME,C_MODULECLASS,C_DISABLE,N_ORDER,N_IMAGEINDEX,C_MODULE_TYPE,C_QUERY_STR,C_REMARK,C_EMP_ID,D_MOD_DT ");
            strSql.Append(" FROM TS_MODULE where  1=1 and C_MODULE_TYPE = '3' and C_DISABLE='1' ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" and C_PARENTMODULEID = '" + strWhere + "' ");
            }
            strSql.Append(" order by N_ORDER asc ");
            return DbHelperOra.Query(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select MAX( substr( C_MODULEID,3))as C_MODULEID ,max(N_ORDER)as N_ORDER  ");
            strSql.Append(" FROM TS_MODULE where  1=1 ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" and C_PARENTMODULEID = '" + strWhere + "' ");
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select C_MODULECLASS FROM TS_MODULE where C_MODULEID='" + C_MODULEID + "' ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select max(c_moduleid) FROM TS_MODULE ");
            object obj = DbHelperOra.GetSingle(strSql.ToString());
            if (obj == null)
            {
                return 0;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from ts_user_fun_pci t where t.c_module_id='" + c_module_id + "' ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select tb.c_moduleclass,tb.c_assemblyname from ts_user_fun_pci ta inner  join ts_module tb on ta.c_module_id = tb.c_moduleid where ta.c_function_type = '2'  and ta.c_user_id = '" + strUserID + "'  and tb.c_parentmoduleid='" + c_parentmoduleid + "' ");

            //strSql.Append(" and tb.c_assemblyname = '" + strFormName + "' ");
