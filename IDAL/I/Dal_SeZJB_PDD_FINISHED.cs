...
{
    /// <summary>
    /// 数据访问类:SeZJB_PDD_FINISHED
    /// </summary>
    public partial class Dal_SeZJB_PDD_FINISHED
	{
		public Dal_SeZJB_PDD_FINISHED()
		{}
		#region  BasicMethod
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SeZJB_PDD_FINISHED ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Mod_SeZJB_PDD_FINISHED GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ysdh,ck,pdrq,djzt,shuser,shdate,finished,ZJBstatus,ts,by1,by2,by3,by4,by5 from SeZJB_PDD_FINISHED ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};

			Mod_SeZJB_PDD_FINISHED model=new Mod_SeZJB_PDD_FINISHED();
		...
		/// 得到一个对象实体
		/// </summary>
		public Mod_SeZJB_PDD_FINISHED DataRowToModel(DataRow row)
		{
			Mod_SeZJB_PDD_FINISHED model=new Mod_SeZJB_PDD_FINISHED();
			if (row != null)
			{
				...
				if(row["djzt"]!=null)
				{
					model.djzt=row["djzt"].ToString();
				}
				if(row["shuser"]!=null)
				{
					model.shuser=row["shuser"].ToString();
				}
				if(row["shdate"]!=null && row["shdate"].ToString()!="")
				{
					model.shdate=DateTime.Parse(row["shdate"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ysdh,ck,pdrq,djzt,shuser,shdate,finished,ZJBstatus,ts,by1,by2,by3,by4,by5 ");
			strSql.Append(" FROM SeZJB_PDD_FINISHED ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM SeZJB_PDD_FINISHED ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from SeZJB_PDD_FINISHED T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SeZJB_PDD_FINISHED";
