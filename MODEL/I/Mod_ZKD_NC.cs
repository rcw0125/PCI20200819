using System;
namespace MODEL
{
    /// <summary>
    /// ReZJB_ZKD:实体类(属性说明自动提取数据库字段的描述信息)
    /// </summary>
    [Serializable]
    public partial class Mod_ZKD_NC
    {
        public Mod_ZKD_NC()
        { }
        #region Model
        private string _zkdh;
        private string _sck;
        private string _tck;
        private string _wlh;
        private string _wlmc;
        private string _pch;
        private string _sx;
        private string _zldj;
        private decimal _jhsl;
        private decimal _jhzl;
        private string _zjldw;
        private string _fjldw;
        private string _ph;
        private string _gg;
        private string _vfree0;
        private string _vfree1;
        private string _vfree2;
        private string _vfree3;
        private string _vfree4;
        private int _pk_zjb_zkd;
        private int? _zjbstatus = 0;
        private string _cappk;
        /// <summary>
        /// 
        /// </summary>
        public string zkdh
        {
            set { _zkdh = value; }
            get { return _zkdh; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string sck
        {
            set { _sck = value; }
            get { return _sck; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string tck
        {
            set { _tck = value; }
            get { return _tck; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string wlh
        {
            set { _wlh = value; }
            get { return _wlh; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string wlmc
        {
            set { _wlmc = value; }
            get { return _wlmc; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string pch
        {
            set { _pch = value; }
            get { return _pch; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string sx
        {
            set { _sx = value; }
            get { return _sx; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string zldj
        {
            set { _zldj = value; }
            get { return _zldj; }
        }
        /// <summary>
        /// 
        /// </summary>
        public decimal jhsl
        {
            set { _jhsl = value; }
            get { return _jhsl; }
        }
        /// <summary>
        /// 
        /// </summary>
        public decimal jhzl
        {
            set { _jhzl = value; }
            get { return _jhzl; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string zjldw
        {
            set { _zjldw = value; }
            get { return _zjldw; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string fjldw
        {
            set { _fjldw = value; }
            get { return _fjldw; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string ph
        {
            set { _ph = value; }
            get { return _ph; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string gg
        {
            set { _gg = value; }
            get { return _gg; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string vfree0
        {
            set { _vfree0 = value; }
            get { return _vfree0; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string vfree1
        {
            set { _vfree1 = value; }
            get { return _vfree1; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string vfree2
        {
            set { _vfree2 = value; }
            get { return _vfree2; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string vfree3
        {
            set { _vfree3 = value; }
            get { return _vfree3; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string vfree4
        {
            set { _vfree4 = value; }
            get { return _vfree4; }
        }
        /// <summary>
        /// 
        /// </summary>
        public int pk_ZJB_ZKD
        {
            set { _pk_zjb_zkd = value; }
            get { return _pk_zjb_zkd; }
        }
        /// <summary>
        /// 
        /// </summary>
        public int? ZJBstatus
        {
            set { _zjbstatus = value; }
            get { return _zjbstatus; }
        }
        /// <summary>
        /// 
        /// </summary>
        public string CAPPK
        {
            set { _cappk = value; }
            get { return _cappk; }
        }
        #endregion Model

    }
}

