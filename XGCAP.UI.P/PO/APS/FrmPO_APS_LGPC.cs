...
{
    public partial class FrmPO_APS_LGPC : Form
    {
        public FrmPO_APS_LGPC()
        {
            InitializeComponent();
        }
        ...
        private void FrmPO_APS_LGPC_Load(object sender, EventArgs e)
        {
           // UserButtonRight.GetBtnFun(this, RV.UI.UserInfo.menuID);
            ...
            //CommonSub.BindIcboNoAll("", "CC", icbo_lz2);
        ...
        private void BindLcLsbGrid(string strCFK)
        {
            ...
            lc_plan_sort = lc_plan.Where(a => a.C_FK == strCFK).OrderBy(a => a.N_SORT).ToList();
        ...
        public int DHL_Left_Cap(DateTime dtHLB, int plan_qua)
        {
            ...
            foreach (var item in results)
            {
                ...
                try
                {
                     cap = Convert.ToInt32(hl_acl.Where(a => a.C_WH_CODE == name).ToList()[0].N_CAP_QUA);
                }
                catch (Exception)
                {
                                    ...
                                    if (lst_lc[lc].C_DFP_HL == "Y")
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                //从没有使用过的缓冷坑找一个坑使用
                                                var lstdhl_act = hl_acl;
                                                try
                                                {
                                                    lstdhl_act = hl_acl.Where(a => a.C_WH_CODE == expectedList[0].C_WH_CODE).ToList();
        ...
        public void UpdateJCByLC(string JC_ID)
        {
            ...
            int xm = 0;
            string remark = "";
            List<Mod_TPP_LGPC_LCLSB> lstlc = bll_lc.GetModelListByJcID(JC_ID);
        ...
        private void btnSaveLcPlan_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < lc_plan.Count; i++)
            {
                Mod_TPP_LGPC_LCLSB mod = lc_plan[i];
                bll_lc.Update(mod);
            }
        ...
        {
            //将没有计划的订单删除
            if (jc_plan.Where(a => a.C_ID == lbl_id.Text && a.N_ORDER_LS == 0).ToList().Count == 1)
            {
                ...
                jc_plan.Remove(jc_plan.Where(a => a.C_ID == lbl_id.Text && a.N_ORDER_LS == 0).ToList()[0]);
            ...
            {
                MessageBox.Show("浇次计划有计划炉次，不能删除！");
                return;
            }
        }
