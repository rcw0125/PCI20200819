...
{
    public partial class FrmPO_Order_Manage : Form
    {
        public FrmPO_Order_Manage()
        {
            InitializeComponent();
        }
        ...
        public void OrderLGAuto(DataTable dt)
        {
            try
            {
                ...
                dtDP.DefaultView.Sort = "  连铸主键, C_BY3, C_RH, 组号 ";
                        ...
                        if (dtDP.Rows[k]["连铸代码"].ToString() == "5#CC" || dtDP.Rows[k]["连铸代码"].ToString() == "7#CC")
                        {
                            ...
                            if (Convert.ToDecimal(dtDP.Rows[k]["整浇次炉数"].ToString()) == 0)//没维护整浇次炉数的数据
                            {
                                ...
                                for (int i = 0; i < n_zjcls; i++)
                                {
                                    ...
                                    mod.C_BY3 = dtDP.Rows[k]["C_BY3"].ToString();
                                    mod.C_HL= dtDP.Rows[k]["C_HL"].ToString();
                                    mod.C_XM = dtDP.Rows[k]["C_XM"].ToString();
                                    mod.C_DFP_HL = dtDP.Rows[k]["C_DFP_HL"].ToString();
                                    mod.C_RH= dtDP.Rows[k]["C_RH"].ToString();
