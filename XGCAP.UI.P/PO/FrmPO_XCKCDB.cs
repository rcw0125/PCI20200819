...
{
    public partial class FrmPO_XCKCDB : Form
    {
        public FrmPO_XCKCDB()
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void FrmPO_XCKCDB_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet dt_areamax = bll_areamax.GetList("");
                imgcbo_XSQY.Properties.Items.Clear();
                foreach (DataRow item in dt_areamax.Tables[0].Rows)//销售区域下拉框绑定
                {
                    imgcbo_XSQY.Properties.Items.Add(item["C_CODE"].ToString(), item["C_ID"], -1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btn_Query_Main_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = bll_roll_product.GetList_RK(txt_BatchNo.Text).Tables[0];
        ...
        private void btn_Query_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = bll_storage.GetList_Query(txt_BatchNo1.Text).Tables[0];
        ...
        private void btn_DB_Click(object sender, EventArgs e)
        {
            try
            {
                if (DialogResult.No == MessageBox.Show("是否确认调拨？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1))
                {
                    return;
                }
                ...
                DialogResult dialogResult = MessageBox.Show("是否确认调拨已选中的数据？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                if (dialogResult == DialogResult.OK)//如果点击“确定”按钮
                {
                    for (int i = 0; i < rownumber.Length; i++)
                    {
                        ...
                        mod.C_LINEWH_LOC_CODE_BEFORE = "";
                        mod.C_FLOOR_BEFORE = "";
                        mod.D_STORAGE_DT = RV.UI.ServerTime.timeNow();
                        mod.C_LINEWH_CODE_AFTER = imgcbo_CK.Text.ToString();
                        mod.C_LINEWH_AREA_CODE_AFTER = imgcbo_QY.Text.ToString();
                        ...
                        if (bll_storage.Add(mod))
                        {
                            ...
                            mod_roll_prodcut.C_MOVE_TYPE = "Z";
                    ...
                    Common.UserLog.AddLog(strMenuName, this.Name, this.Text, "线材调拨");//添加操作日志
                    MessageBox.Show("调拨成功！");
        ...
        private void btn_XS_Click(object sender, EventArgs e)
        {
            try
            {
                if (DialogResult.No == MessageBox.Show("是否确认销售出库？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1))
                {
                    return;
                }
                int[] rownumber = gv_SJXX.GetSelectedRows();//获取选中行号数组；
                if (rownumber.Length == 0) return;
                if (imgcbo_XSQY.EditValue == null)
                {
                    MessageBox.Show("销售区域不能为空！");
                    return;
                }
                if (imgcbo_YSFS.EditValue == null)
                {
                    MessageBox.Show("运输方式不能为空！");
                    return;
                }
                if (DialogResult.No == MessageBox.Show("是否确认销售出库已选中的数据？", "提示"))
                {
                    return;
                }
                for (int i = 0; i < rownumber.Length; i++)
                {
                    ...
                    mod.C_SALE_AREA = imgcbo_XSQY.Text.ToString();
                    mod.C_TRANSPORTATION = imgcbo_YSFS.Text.ToString();
                    mod.C_EMP_ID = RV.UI.UserInfo.UserID;
                    mod.D_MOD_DT = RV.UI.ServerTime.timeNow();
                    if (bll_storage.Add(mod))
                    {
                        Mod_TRC_ROLL_PRODCUT mod_roll_prodcut = bll_roll_product.GetModel(dt.Rows[0]["C_ID"].ToString());
                        mod_roll_prodcut.C_SALE_AREA = imgcbo_XSQY.Text.ToString();
                        mod_roll_prodcut.C_MOVE_TYPE = "S";
                        mod_roll_prodcut.C_EMP_ID = RV.UI.UserInfo.UserID;
                        mod_roll_prodcut.D_MOD_DT = RV.UI.ServerTime.timeNow();
                        bll_roll_product.Update(mod_roll_prodcut);
                        ...
                        Common.UserLog.AddLog(strMenuName, this.Name, this.Text, "线材销售出库");//添加操作日志
                    }
                }
            }
            catch (Exception ex)
