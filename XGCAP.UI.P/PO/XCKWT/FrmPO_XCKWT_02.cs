...
{
    public partial class FrmPO_XCKWT_02 : Form
    {
        public FrmPO_XCKWT_02()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dt = bll_TRC_Roll_Product.GetList_RK_KW(BatchNo, kw, "", "602").Tables[0];
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    foreach (var item in this.pnlMain.Controls)
                    {
                        if (item is DevExpress.XtraEditors.LabelControl)
                        {
                            ...
                            {
                                DataTable dt_con = bll_TRC_Roll_Product.GetList_RK_KW("", lbla.AccessibleDescription,"","602").Tables[0];
