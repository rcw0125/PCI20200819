...
{
    public partial class FrmPO_XCKWT_10_33_Z_BEI : Form
    {
        public FrmPO_XCKWT_10_33_Z_BEI()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dt = bll_TRC_Roll_Product.GetList_RK_KW(BatchNo, kw, "61033", "610").Tables[0];
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    foreach (var item in this.pnlMain.Controls)
                    {
                        if (item is DevExpress.XtraEditors.LabelControl)
                        {
                            ...
                            {
                                DataTable dt_con = bll_TRC_Roll_Product.GetList_RK_KW("", lbla.AccessibleDescription, "61033", "610").Tables[0];
