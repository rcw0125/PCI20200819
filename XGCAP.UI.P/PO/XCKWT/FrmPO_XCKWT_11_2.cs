...
{
    public partial class FrmPO_XCKWT_11_2 : Form
    {
        public FrmPO_XCKWT_11_2()
        {
            InitializeComponent();
        }
        ...
        {
            DataTable dt = bll_TRC_Roll_Product.GetList_RK_KW(BatchNo, kw,"61102","611").Tables[0];
        ...
        private void FrmPO_XCKWT_Load(object sender, EventArgs e)
        {
            ...
            foreach (var item in this.pnlMain.Controls)
            {
                if (item is DevExpress.XtraEditors.LabelControl)
                {
                    ...
                    {       
                        DataTable dt_1_10 = bll_TRC_Roll_Product.GetList_LXC("61102", str[0], str[1]).Tables[0]; 
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    foreach (var item in this.pnlMain.Controls)
                    {
                        if (item is DevExpress.XtraEditors.LabelControl)
                        {
                            ...
                            {
                                DataTable dt_con = bll_TRC_Roll_Product.GetList_RK_KW("", lbla.AccessibleDescription, "61102", "611").Tables[0];
        ...
        private void labelControl52_Click(object sender, EventArgs e)
        {
            ...
            string kw = lab_id.AccessibleDescription;
            string[] str = kw.Split(',');
            DataTable dt = bll_TRC_Roll_Product.GetList_LXC("61102", str[0], str[1]).Tables[0];
            gc_SJXX.DataSource = dt;
            gv_SJXX.BestFitColumns();
            SetGridViewRowNum.SetRowNum(gv_SJXX);
        }
