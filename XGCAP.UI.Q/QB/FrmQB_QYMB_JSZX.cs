...
{
    public partial class FrmQB_QYMB_JSZX : Form
    {
        private Bll_TQB_SAMP_MODLE_JSZX bllTqbSampModleJszx = new Bll_TQB_SAMP_MODLE_JSZX();
        private Bll_TQB_SAMP_ITEM_MODLE_JSZX bllTqbSampItemModleJszx = new Bll_TQB_SAMP_ITEM_MODLE_JSZX();
        private Bll_TQB_STD_MAIN bllMain = new Bll_TQB_STD_MAIN();
        private Bll_TQB_SAMPLING bllTqbSam = new Bll_TQB_SAMPLING();

        public FrmQB_QYMB_JSZX()
        {
            InitializeComponent();
        }

        private void FrmQB_QYMB_JSZX_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = bllTqbSampItemModleJszx.Get_Model_List(dr["执行标准"].ToString(), dr["钢种"].ToString(), dr["规格最小值"].ToString(), dr["规格最大值"].ToString()).Tables[0];
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (dtMain.Rows.Count > 0)
                        {
                            ...
                            Mod_TQB_SAMP_MODLE_JSZX modSam = new Mod_TQB_SAMP_MODLE_JSZX();
                            modSam.C_ID = i_id.ToString();
                            modSam.C_STD_CODE = dt.Rows[i]["文件号"].ToString().Trim();
                            modSam.C_STL_GRD = dt.Rows[i]["牌号"].ToString().Trim();
                            //modSam.C_SPEC_MIN = dt.Rows[i]["规格"].ToString().Trim();
                            //modSam.C_SPEC_MAX = dt.Rows[i]["规格最大"].ToString().Trim();

                            bllTqbSampModleJszx.Add(modSam);

                            for (int kk = 3; kk < dt.Columns.Count; kk++)
                            {
                                string strID = bllTqbSam.GetSampId(dt.Columns[kk].ColumnName,"技术中心");
                                ...
                                if (!string.IsNullOrEmpty(dt.Rows[i][kk].ToString().Trim()))
                                {
                                    ...
                                    bllTqbSampItemModleJszx.Add(modItem);
                                }
        ...
        private void btn_Add_Main_Click(object sender, EventArgs e)
        {
            try
            {
                FrmQB_QYMB_Add_MAIN_JSZX frm = new FrmQB_QYMB_Add_MAIN_JSZX();
        ...
        private void btn_Del_Main_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dr != null)
                {
                    if (bllTqbSampModleJszx.update_status(dr["执行标准"].ToString(), dr["钢种"].ToString(), dr["规格最小值"].ToString(), dr["规格最大值"].ToString()))
                    {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        string result = bllTqbSampModleJszx.Save_Modle(dt);
        ...
        private void btn_Del_Item_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dr != null)
                {
                    if (bllTqbSampItemModleJszx.Update_Trans(dr["C_ID"].ToString()))
                    {
                        MessageBox.Show("删除成功！");
