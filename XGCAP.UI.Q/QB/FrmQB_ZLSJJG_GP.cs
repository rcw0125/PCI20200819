...
{
    public partial class FrmQB_ZLSJJG_GP : Form
    {
        private Bll_TQD_DESIGN bllDesign = new Bll_TQD_DESIGN();

        public FrmQB_ZLSJJG_GP()
        {
            InitializeComponent();
        }
        int a = 0;
        ...
        private void NewMethod_CF()
        {
            ...
            DataTable dt_CX = bll_Design_Item.GetList_Query(dr["C_DESIGN_NO"].ToString()).Tables[0];
        ...
        private void GetDesignLog_Add()
        {
            if (a == 0)
            {
                DataRow dr_sj = this.gv_GPSJ.GetDataRow(this.gv_GPSJ.FocusedRowHandle);
