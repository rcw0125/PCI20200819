...
{
    public partial class FrmQC_JSSY_COPY_NEW : Form
    {
        ...
        public FrmQC_JSSY_COPY_NEW()
        {
            InitializeComponent();
        }

        private void FrmQC_JSSY_COPY_NEW_Load(object sender, EventArgs e)
        {
        ...
        private void BindSYXX_CG()
        {
            try
            {
                ...
                gv_SYXX.Columns[14].Visible = false;
            ...
            try
            {
                ...
                for (int i = 0; i < gv_SYXX.DataRowCount; i++)
                {
                    ...
                    if (val == "True")
                    {
                        ...
                        for (int k = 15; k < gv_SYXX.Columns.Count; k++)
                        {
                            if (!string.IsNullOrEmpty(gv_SYXX.GetRowCellValue(i, gv_SYXX.Columns[k].FieldName).ToString()))
                            {
                                int result = 0;
                                if (!int.TryParse(gv_SYXX.GetRowCellValue(i, gv_SYXX.Columns[k].FieldName).ToString(), out result))
                                {
                                    failNum++;
        ...
        private void btn_Match_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dtMain.Rows.Count; i++)
            {
                if (strBatch != dtMain.Rows[i]["批号"].ToString())
                {
                    ...
                    rows = dt.Select(" 执行标准='" + dtMain.Rows[i]["执行标准"].ToString() + "' and 钢种='" + dtMain.Rows[i]["钢种"].ToString() + "' ");
                ...
                if (rows.Length > 0)
                {
                    if (mm < rows.Length)
                    {
                        ...
                        {
                            for (int kk = 15; kk < dtMain.Columns.Count; kk++)
                            {
