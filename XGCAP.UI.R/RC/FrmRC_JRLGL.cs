...
{
    public partial class FrmRC_JRLGL : Form
    {
        public FrmRC_JRLGL()
        {
            InitializeComponent();
        ...
        private void FrmRC_JRLGL_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private DataTable Query1()
        {
            ...
            if (!string.IsNullOrWhiteSpace(this.txt_Std.Text))
                whereSql += " AND TRM.C_STD_CODE LIKE '%" + txt_Std.Text + "%'  ";

            if (!string.IsNullOrWhiteSpace(this.txt_Spec.Text))
                whereSql += " AND TRM.C_SPEC LIKE '%" + txt_Spec.Text + "%'  ";

            DataTable dt = bll_TRC_ROLL_MAIN.GetListMain(whereSql, true).Tables[0];
        ...
        private DataTable Query2()
        {
            ...
            DataTable dt = bll_TRC_ROLL_MAIN.GetList(whereSql, true, 1).Tables[0];
        ...
        private DataTable Query3()
        {
            ...
            DataTable dt = bll_TRC_ROLL_MAIN.GetList(whereSql, true, 2).Tables[0];
        ...
        private void btn_PutBack_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //撤回支数不能大于入炉支数
                int furnaceNum = bll_TRC_ROLL_MAIN.GetFurnaceNum(staID, int.Parse(num), 1);
                ...
                if (bll_TRC_ROLL_MAIN.PutBackHandler(staID, this.txt_PutRemark.Text, int.Parse(num)) == "1")
        ...
        private void btn_PutElim_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (bll_TRC_ROLL_MAIN.PutElimHandler(staID, this.txt_PutRemark.Text, int.Parse(putNum), slabwhCode, shift, group, userID, id) == "1")
        ...
        private void btn_OutFurnace_Click(object sender, EventArgs e)
        {
            ...
            do
            {
                ...
                this.txt_OutRemark.Text = dt.Rows[0]["C_REMARK"] == null ? "" : dt.Rows[0]["C_REMARK"].ToString();
                string id = dt.Rows[0]["C_ID"].ToString();
                string batchNo = dt.Rows[0]["C_BATCH_NO"].ToString();
                string staID = dt.Rows[0]["C_STA_ID"].ToString();
                if (bll_TRC_ROLL_MAIN.OutFurnaceHandler(id, num, batchNo, staID, this.cbo_OutShift.EditValue.ToString(), this.cbo_OutGroup.EditValue.ToString(), this.txt_OutRemark.Text, RV.UI.ServerTime.timeNow()) == "1")
                    result++;
        ...
        private void btn_OutElim_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (bll_TRC_ROLL_MAIN.OutElimHandler(staID, this.txt_OutRemark.Text, int.Parse(outNum), slabwhCode, shift, group, userID) == "1")
                    MessageBox.Show("剔除成功！");
                else
                    MessageBox.Show("剔除失败！");
