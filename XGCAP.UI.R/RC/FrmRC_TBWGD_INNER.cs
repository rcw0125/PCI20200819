...
{
    public partial class FrmRC_TBWGD_INNER : Form
    {
        public FrmRC_TBWGD_INNER()
        {
            InitializeComponent();
        ...
        private void FrmRC_TBWGD_INNER_Load(object sender, EventArgs e)
        {
            BindWgdFinished();
            BindWgd();
        }
        ...
        public void BindWgd()
        {
            string sqlWhere = @"   N_SCRF=2  
                                                  ...
                                                  AND  C_STA_ID='" + staID + "'      AND  N_IS_MERGE=1    ";
        ...
        private void btn_SyncRF_Click(object sender, EventArgs e)
        {
            if (dtRF != null && dtRF.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < 1; i++)
                {
                    dal_Interface_FR.TbKCList2("", "", "", dtRF.Rows[i]["WGDH"].ToString(), "", "", "");
            ...
            try
            {
                ...
                if (dtNC != null && dtNC.Rows.Count > 0)
                {
                    for (int i = 0; i < dtNC.Rows.Count; i++)
                    {
                        ...
                        var plan = bllPlanRollItemInfo.GetModel(wgdItemInfo.C_ITEM_INFO_ID);
                        ...
                        ncA1.zyx5 = plan.C_PLAN_ROLL_ID;
                        ncA1.jhfaid = matrl.C_PK_PSID;
                        ncA1.memo = plan.C_AREA;
                        string a1Name = plan.C_PLAN_ROLL_ID + "_A1.xml";
                        ...
                        ncA2.freeitemvalue5 = plan.C_PLAN_ROLL_ID;
