...
{
    public partial class FrmRF_TBTKXX : Form
    {
        public FrmRF_TBTKXX()
        {
            InitializeComponent();
        ...
        private void btn_tb_Click(object sender, EventArgs e)
        {
            ...
            if (grdrow.Count() > 0)
            {
                ...
                foreach (var grditem in grdrow)
                {
                    ...
                    if (mod_pro.C_MOVE_TYPE=="S"|| mod_pro.C_MOVE_TYPE == "QS")
                    {
                        ...
                        res = res + 1;
                        Common.UserLog.AddLog(str_meunname, this.Name, this.Text, "同步条码退库信息，同步批号："+ PCH+"；钩号："+ GH+"！");//添加操作日志
                    }
